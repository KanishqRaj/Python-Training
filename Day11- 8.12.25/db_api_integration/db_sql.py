...
def innerjoin():
    logging.debug(f"INNER JOIN /employees/ called")
    try:
        db = get_db()
        cursor = db.cursor()

        sql = "select e.name,e.role , e.salary, d.id, d.dept_name, d.location from employees e inner join department d on e.role = d.dept_name"
        ...
        logging.debug("INNER JOIN completed")
        return "Inner join executed"
    except Exception as e:
